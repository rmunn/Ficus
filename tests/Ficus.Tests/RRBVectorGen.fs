...
let maxSlots childCount =
    childCount
    * Literals.blockSize

let genSlotCount childCount =
    Gen.choose (minSlots childCount, maxSlots childCount)

let distribute slots childCount =
    gen {
        ...
        let mutable availableBucketIndices = [
            0 .. childCount
                 - 1
        ]
        ...
        while remaining > 0 do
            ...
            if
                ...
                >= Literals.blockSize
            then
                availableBucketIndices <-
                    availableBucketIndices
                    |> List.filter ((<>) chosenIdx)
...
let rec genSpecificRRBNode (g: Gen<'T>) level (childSizes: ListItem) =
    gen {
        let childrenGenerators =
            ...
            | List items ->
                items
                |> List.map (genSpecificRRBNode g (level - 1))
...
let mkSpecificRRBTwig itemSrc (childSizes: ListItem) =
    ...
    | List items when
        items
        |> List.forall isInt
        -> // This also succeeds on empty root node, which is good
        items
        |> Array.ofList
        |> Array.map (
            function
...
let rec mkSpecificRRBNode itemSrc level (childSizes: ListItem) =
    if level = 1 then
        match childSizes with
        | Int n ->
            failwith "Individual ints should have been handled as part of their component lists"
        | List items when
            items
            |> List.forall isInt
            -> // This also succeeds on empty root node, which is good
            mkSpecificRRBTwig itemSrc childSizes
        | List _ -> failwith "Shouldn't be at level 1 here"
    else
        match childSizes with
...
let strJoin (between: string) (parts: #seq<string>) = System.String.Join(between, parts)

let sizeToTreeReprStr =
    function
    | i when
        i
        >= Literals.blockSize
...
let rec nodeToTreeReprStr<'T> level (node: RRBNode<'T>) =
    if level > 1 then
        ...
        let children =
            ...
            |> Seq.map (fun node -> nodeToTreeReprStr (level - 1) node)
            |> List.ofSeq
        ...
        if
            leaves
            |> List.distinctBy (fun leaf -> leaf.NodeSize)
            |> List.length = 1
        then
...
let vecToTreeReprStr (vec: RRBVector<'T>) =
    ...
    then
        ...
        let rootRepr =
            ...
            |> fun s ->
                s.[1 .. s.Length
                        - 2] // Strip brackets from outermost list

        (sprintf "%s T%d" rootRepr tailLen).Trim()

let genSpecificTree g treeRepr = failwith "Not implemented yet" // TODO: Write if desired

let rec genRRBNode (g: Gen<'a>) level childCount =
    gen {
        ...
        let childrenGenerators =
            if level > 1 then
                childSizes
                |> Array.map (genRRBNode g (level - 1))
            else
                childSizes
                |> Array.map (fun n ->
                    g
...
let rec genFullNode (g: Gen<'a>) level childCount =
    gen {
        ...
        let childrenGenerators =
            if level > 1 then
                childSizes
                |> Array.map (genFullNode g (level - 1))
            else
                childSizes
                |> Array.map (fun n ->
                    g
...
let genNode (g: Gen<'a>) level childCount =
    Gen.frequency [
        3, genRRBNode g level childCount
        ...
        then
            ...
            let! newLeaf =
                gen {
                    ...
                    return
                        (Array.append (leaf :?> RRBLeafNode<'a>).Items newItems)
                        |> mkLeaf
                }
...
let genVec<'a> level childCount =
    if level <= 0 then
        failwith "genVec needs to have level 1+. For level 0, use genTinyVec"

    if childCount < 2 then
        failwith "genVec needs to have childCount 2+. For level 0, use genTinyVec"
    ...
    gen {
        ...
        let! root' =
            (root :?> RRBFullNode<'a>)
            |> fleshOutRightmostLeafIfNecessary g level
        ...
        let vec =
            RRBPersistentVector<'a>(totalSize, shift, root', tail, rootSize) :> RRBVector<'a>
...
let sizedGenVec<'a> =
    ...
    <| fun s ->
        gen {
            ...
            let len =
                (max len 0) % (Literals.blockSize
                               * 5) // Ensure length is reasonable

            let! vec = genVecOfLength<'a> len
            let! coinFlip = Arb.generate<bool>
            return (if coinFlip then vec else vec.Transient())
        // return vec
        }
...
let genLeafRepr size =
    Gen.constant (
        size
        |> ListItem.Int
    )

let rec genNodeReprImpl level childSizes g =
    gen {
        let childrenGenerators =
            if level > 1 then
                childSizes
...
let rec genRelaxedNodeRepr level childCount =
    gen {
        let! slotCount = genSlotCount childCount
        let! childSizes = distribute slotCount childCount
...
let rec genFullNodeRepr level childCount =
    gen {
        let childSizes = Array.create childCount Literals.blockSize
...
let genNodeRepr level childCount =
    Gen.frequency [
        3, genRelaxedNodeRepr level childCount
...
let shrinkNodeRepr node =
    ...
    | Int n ->
        Arb.shrink n
        |> Seq.filter (fun n -> n > 0 (* Empty leaves are not allowed *) )
        |> Seq.map Int
    | List l ->
        Arb.shrink l
        |> Seq.filter (
...
let fleshOutRightmostLeafOfRepr (root: ListItem) =
    ...
    | Some leafList ->
        ...
        if
            len <= 0
            || leaves.[len - 1] = Int Literals.blockSize
        then
            root
        else
            let isFull =
                leaves
                |> Seq.take (len - 1)
                |> Seq.forall ((=) (Int Literals.blockSize))
...
let shrinkTreeRoot root =
    shrinkNodeRepr root
    |> Seq.map fleshOutRightmostLeafOfRepr

let shrinkTreeRepr (tree: TreeRepresentation) =
    let roots =
        shrinkTreeRoot tree.Root
        |> Seq.map (fun root -> { tree with Root = root })

    let tailSizes =
        match tree.Tail with
        | None -> Seq.empty
    ...
    let tails =
        ...
        |> Seq.map (fun size -> { tree with Tail = Some size })
...
let genTreeReprOfLength len =
    ...
    then
        Gen.constant {
            Root = List [ Int Literals.blockSize ]
            Tail =
                Some(
                    len
                    - Literals.blockSize
...
let genListForRunTesting =
    Gen.sized (fun n ->
        let boundedSize =
            n
            |> min Literals.blockSize
            |> max 2
