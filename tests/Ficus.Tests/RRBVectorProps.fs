...
let rec height (node: RRBNode<'T>) =
    if isLeaf node then
        0
    elif isEmpty node then
        1 // Empty roots are considered at height 1 now
    else
        1
...
type Fullness =
    | CompletelyFull
    | FullEnough
    | NotFull // Used in node properties
...
let nodeProperties = [
    ...
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec check shift (node: RRBNode<'T>) =
            ...
            then
                ...
                && isNotTwig node
                && children node
                   |> Seq.forall (fun n -> check (down shift) (n :?> RRBFullNode<'T>))
    ...
    "All leaves should be at height 0",
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec check shift node =
            if isLeaf node then
                ...
                not (isEmpty node)
                && children node
                   |> Seq.forall (check (down shift))

        check shift root

    "The number of items in each node and leaf should be <= the branching factor (Literals.blockSize)",
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec check shift (node: RRBNode<'T>) =
            if shift <= 0 then
                node.NodeSize
    ...
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec check shift (node: RRBNode<'T>) =
            ...
            then
                node
                |> itemCount shift = node.TreeSize shift
            else
                node
                |> itemCount shift = node.TreeSize shift
                && children node
    ...
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec check shift (node: RRBNode<'T>) =
            if shift <= 0 then
                node.NodeSize = node.TreeSize shift
            else
                children node
                |> Seq.forall (check (down shift))
    ...
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec check shift (node: RRBFullNode<'T>) =
            ...
            let expectedSizeTable =
                children node
                |> Seq.map (fun child -> child.TreeSize(down shift))
                |> Seq.scan (+) 0
                |> Seq.skip 1
                |> Array.ofSeq
    ...
    "The size table of any tree node should have as many entries as its # of direct child nodes",
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec check (node: RRBFullNode<'T>) =
            if isRelaxed node then
                Array.length (node :?> RRBRelaxedNode<'T>).SizeTable = Array.length node.Children
            else
                true

        check (root :?> RRBFullNode<'T>)

    "A full node should never contain less-than-full children except as its last child",
    fun (shift: int) (root: RRBNode<'T>) ->
        // Rules:
        ...
        let rec fullness shift (node: RRBNode<'T>) =
            if shift <= 0 then
                ...
                let fullnessResult =
                    ...
                    then
                        if (fst last) = CompletelyFull then
                            CompletelyFull
                        else
                            FullEnough
                    else
    ...
    "Internal nodes that are at shift > 0 should never be leaves",
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec check shift node =
            if shift > 0 then
                node
    ...
    "A tree should not contain any empty nodes",
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec check shift node =
            node
            |> (not
    ...
    "No RRBRelaxedNode should contain a \"full\" size table. If the size table was full, it should have been turned into an RRBFullNode.",
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec check shift (node: RRBNode<'T>) =
            if shift <= 0 then
                true
    ...
    "The shift of a vector should always be a multiple of Literals.shiftSize",
    fun (shift: int) (root: RRBNode<'T>) -> shift % Literals.shiftSize = 0

    "The shift of a vector should always be the height from the root to the leaves, multiplied by Literals.shiftSize",
    fun (shift: int) (root: RRBNode<'T>) ->
        let rec height acc (node: RRBNode<'T>) =
            if isLeaf node then
                acc
            else
                (node :?> RRBFullNode<'T>).FirstChild
                |> height (acc + 1)

        (height 0 root)
        * Literals.shiftSize = shift

    "ExpandedNodes (and ExpandedRRBNodes) should not appear in a tree whose root is not an expanded node variant",
    fun (shift: int) (root: RRBNode<'T>) ->
        let isExpanded (child: RRBNode<'T>) =
            (child :? RRBExpandedFullNode<'T>)
            || (child :? RRBExpandedRelaxedNode<'T>)

        let rec check shift (node: RRBNode<'T>) =
            if shift <= 0 then
                true
            elif isExpanded node then
                false
            else
                children node
                |> Seq.forall (check (down shift))
    ...
    "If a tree's root is an expanded Node variant, its right spine should contain expanded nodes but nothing else should",
    fun (shift: int) (root: RRBNode<'T>) ->
        let isExpanded (child: RRBNode<'T>) =
            (child :? RRBExpandedFullNode<'T>)
            || (child :? RRBExpandedRelaxedNode<'T>)
        ...
        let rec check shift isLast (node: RRBNode<'T>) =
            ...
            then
                ...
                let checkResultForAllButLast =
                    children node
                    |> Seq.take (
                        node.NodeSize
                        - 1
                    )
                    |> Seq.forall (check (down shift) false)

                let checkResultForLastChild =
                    children node
                    |> Seq.skip (
                        node.NodeSize
                        - 1
                    ...
                    |> Seq.head
                    |> check (down shift) isLast

                checkResultForAllButLast
                && checkResultForLastChild

        if isExpanded root then check shift true root else true

    "ExpandedNodes (and ExpandedRRBNodes) should have exactly as much data in their children & size tables as their node size indicates",
    fun (shift: int) (root: RRBNode<'T>) ->
        let isExpanded (child: RRBNode<'T>) =
            (child :? RRBExpandedFullNode<'T>)
            || (child :? RRBExpandedRelaxedNode<'T>)
        ...
        let rec check shift (node: RRBNode<'T>) =
            if shift <= 0 then
                true
            else if isExpanded node then
                let sizeTableClean =
                    if isRelaxed node then
                        (node :?> RRBExpandedRelaxedNode<'T>).SizeTable
                        |> Seq.skip node.NodeSize
                        |> Seq.forall ((=) 0)
                        && (node :?> RRBExpandedRelaxedNode<'T>).SizeTable
                           |> Seq.take node.NodeSize
                           |> Seq.forall ((<>) 0)
                    else
                        true
                ...
                let childrenArrayClean =
                    if isRelaxed node then
                        (node :?> RRBExpandedRelaxedNode<'T>).Children
                        |> Seq.skip node.NodeSize
                        |> Seq.forall isNull
                        && (node :?> RRBExpandedRelaxedNode<'T>).Children
                           |> Seq.take node.NodeSize
                        ...
                        (node :?> RRBExpandedFullNode<'T>).Children
                        |> Seq.skip node.NodeSize
                        |> Seq.forall isNull
                        && (node :?> RRBExpandedFullNode<'T>).Children
                           |> Seq.take node.NodeSize
...
let internal vectorPropertiesPersistent = [
    "The total number of items in all leaf nodes combined, plus the tail, should equal the vector length",
    fun (vec: RRBPersistentVector<'T>) ->
    ...
    "Unless the vector is empty, the tail should never be empty",
    fun (vec: RRBPersistentVector<'T>) ->
        let tailLen = vec.Tail.Length
        if vec.Length > 0 then tailLen > 0 else tailLen = 0

    "The vector's length, shift, and tail offset must not be negative",
    fun (vec: RRBPersistentVector<'T>) ->
        vec.Length
        >= 0
        && vec.Shift
    ...
    "The number of items in the tail should be <= the branching factor (Literals.blockSize)",
    fun (vec: RRBPersistentVector<'T>) ->
        let tailLen = vec.Tail.Length
    ...
    "If the vector shift is > Literals.shiftSize, then the root node's length should be at least 2",
    fun (vec: RRBPersistentVector<'T>) -> // A true vector property because only by calling vector methods can it be achieved
        if vec.Shift > Literals.shiftSize then
            vec.Root.NodeSize
            >= 2
        else
            true

    "The tail offset should equal (vector length - tail length) at all times, even when the vector is empty",
    fun (vec: RRBPersistentVector<'T>) ->
        let tailLen = vec.Tail.Length

        vec.TailOffset = vec.Length
                         - tailLen

    "The tail offset should equal (root.TreeSize this.Shift) at all times, unless the tail offset is 0",
    fun (vec: RRBPersistentVector<'T>) ->
        if vec.TailOffset = 0 then
            true
        else
            vec.TailOffset = vec.Root.TreeSize vec.Shift

    "The root node should be empty if (and only if) the tail offset is 0 (i.e., it's a tail-only node)",
    fun (vec: RRBPersistentVector<'T>) -> (vec.TailOffset = 0) = (vec.Root.NodeSize = 0)

    "If tail offset = 0, the vector length should be blockSize or less",
    fun (vec: RRBPersistentVector<'T>) ->
        if vec.TailOffset > 0 then
            true
        else
    ...
    "The shift of a vector should always be a multiple of Literals.shiftSize",
    fun (vec: RRBPersistentVector<'T>) -> vec.Shift % Literals.shiftSize = 0

    "The shift of a vector should always be the height from the root to the leaves, multiplied by Literals.shiftSize",
    fun (vec: RRBPersistentVector<'T>) ->
        vec.Shift = (height vec.Root)
                    * Literals.shiftSize

    "If a tree is Persistent, its root should NOT be an Expanded node variant",
    fun (vec: RRBPersistentVector<'T>) ->
        let isExpanded (node: RRBNode<'T>) =
            (node :? RRBExpandedFullNode<'T>)
            || (node :? RRBExpandedRelaxedNode<'T>)
...
let internal vectorPropertiesTransient = [
    ...
    "Transients' tails should be full at all times",
    fun (vec: RRBTransientVector<'T>) -> vec.Tail.Length = Literals.blockSize

    "Transients' tails should have exactly (vec.Length - vec.TailOffset) real items, and the rest should equal Unchecked.defaultof<'T>",
    fun (vec: RRBTransientVector<'T>) ->
        let tailLen =
            vec.Length
            - vec.TailOffset
        ...
        for i = tailLen to Literals.blockSize
                           - 1 do
            result <-
                result
                && (vec.Tail.[i] = Unchecked.defaultof<'T>)
    ...
    "The rightmost leaf node of the vector should always be full if its parent is full",
    fun (vec: RRBTransientVector<'T>) -> // A true vector property because only by calling vector methods can it be achieved
        if vec.TailOffset = 0 then
            true
        else
            let rec getRightTwig shift (node: RRBFullNode<'T>) =
            ...
            if isFull twig then
                twig.LastChild.NodeSize = Literals.blockSize
            else
                true
    ...
    "The root node should be empty if (and only if) the tail offset is 0 (i.e., it's a tail-only node)",
    fun (vec: RRBTransientVector<'T>) -> (vec.TailOffset = 0) = (vec.Root.NodeSize = 0)
    ...
    "The shift of a vector should always be a multiple of Literals.shiftSize",
    fun (vec: RRBTransientVector<'T>) -> vec.Shift % Literals.shiftSize = 0
    ...
    "If a tree is Transient, its root should be an Expanded node variant",
    fun (vec: RRBTransientVector<'T>) ->
        let isExpanded (node: RRBNode<'T>) =
            (node :? RRBExpandedFullNode<'T>)
            || (node :? RRBExpandedRelaxedNode<'T>)
...
let checkNodeProperty name pred shift root =
    ...
    | :? System.InvalidCastException -> [
        "Invalid cast while checking "
        + name
      ]
    | :? System.IndexOutOfRangeException -> [
        "Index out of range while checking "
        + name
      ]
...
let getNodePropertyResults shift root =
    nodeProperties
    |> List.collect (fun (name, pred) -> checkNodeProperty name pred shift root)
...
let getVecPropertyResults props vec =
    props
    |> List.collect (fun (name, pred) -> checkVectorProperty name pred vec)

let getVecAndNodePropertyResults props shift (root: RRBNode<'T>) vec =
    let vecProps = getVecPropertyResults props vec

    let nodeProps =
        if root.NodeSize > 0 then
            getNodePropertyResults shift root
        else
            []
...
let getAllPropertyResults (vec: RRBVector<'T>) =
    match vec with
    | :? RRBPersistentVector<'T> as v ->
        getVecAndNodePropertyResults vectorPropertiesPersistent v.Shift v.Root v
    | :? RRBTransientVector<'T> as v ->
        getVecAndNodePropertyResults vectorPropertiesTransient v.Shift v.Root v
