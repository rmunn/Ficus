...
[<EntryPoint>]
let main argv =
    // TODO: Let Expecto handle args now
    if
        argv
        |> Seq.contains ("--version")
    then
    ...
    then
        ...
        argv
        |> Array.contains "--stress"
        || argv
           |> Array.contains "--fscheck-only"
    then
        printfn
            "Running only FsCheck tests%s"
            (if
                 argv
                 |> Array.contains "--stress"
             then
        ...
        let rec containsTests =
            function
            | TestCase _ -> true
            | TestList([], _) -> false
            | TestList(tests, _) ->
                tests
                |> List.exists containsTests
