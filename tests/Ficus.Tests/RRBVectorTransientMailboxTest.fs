...
let internal checkOrigVec (vec: RRBPersistentVector<'a>) (arr: 'a[]) =
    ...
    then
        failwithf
            "Original vector and its original array should be same length, but vector length was %d and array length was %d. Vec: %A and array: %A"
            vec.Length
            arr.Length
            vec
            arr
    ...
    for i = 0 to maxIdx do
        ...
        then
            failwithf
                "Original vector was modified at index %d! This was probably caused by a thread. vec.[%d] = %A and arr.[%d] = %A"
                i
                i
                e.Current
                i
...
let internal completionAgent
    size
    (cts: CancellationTokenSource)
    (parent: MailboxProcessor<AllThreadsResult>)
    =
...
let internal transientAgent
    token
    (completionAgent: MailboxProcessor<SingleThreadResult>)
    (position: int)
    (tvec: RRBTransientVector<int>)
    =
            ...
            async {
                let! msg =
                    ...
                    with :? System.TimeoutException ->
                        ...
                        async { return Finished } // If we're waiting five seconds without getting anything, give up and call this thread finished
                    ...
                    if shouldRun then
                        ...
                        with e ->
                            do!
                                logger.infoWithBP (
                                    eventX
                                        "During {cmd} execution (vec was {vec}), error after {cmdsDone} commands in thread {pos} with message {msg}"
                            ...
                            Failed(position, cmdsDone, vec, arr, Some cmd, e.Message)
                            |> completionAgent.Post
                        ...
                        try
                            ...
                            if not success then
                                Failed(position, cmdsDone, vec, arr, Some cmd, msg)
                                |> completionAgent.Post
                        ...
                        | e ->
                            do!
                                logger.infoWithBP (
                                    eventX
                                        "During {cmd} post checking (vec was {vec}), error after {cmdsDone} commands in thread {pos} with message {msg}"
                                    >> setField "cmd" (sprintf "%A" cmd)
                                    >> setField "vec" (sprintf "%A" vec)
                                    >> setField "cmdsDone" cmdsDone
                                    >> setField "pos" position
...
let internal startSplitTesting (vec: RRBPersistentVector<int>) (cmds: (Cmd list)[]) =
    ...
    let eq, msg =
        (origArr = (tvec
                    |> RRBVector.toArray)),
        "Initial transient didn't equal initial persistent"
    ...
    if
        ...
        <> cmds.Length
    then
        failwith "Pass as many cmds as expected splits" // TODO: Populate error msg with some data here
    ...
    let run (mailbox: MailboxProcessor<AllThreadsResult>) =
        async {
            ...
            | Go channel ->
                ...
                let completion =
                    mailbox
                    |> completionAgent parts.Length cts

                let threads =
                    parts
                    |> Array.mapi (transientAgent cts.Token completion)
                // Send all commands to their respective threads as quickly as possible
                (cmds, threads)
                ||> Array.iteri2 (fun position cmdList thread ->
                    thread.Error.Add(fun e ->
                        completion.Post(
                            Failed(
                                position,
                                -1,
                                RRBTransientVector<int>.MkEmpty(),
                                Array.empty,
                                None,
                                e.ToString()
                            )
            ...
            while mailbox.CurrentQueueLength = 0
                  && not (cts.IsCancellationRequested) do
            ...
            let! msg =
                try
                    mailbox.Receive(45000)
                with :? TimeoutException ->
                    Expecto.Tests.failtest
                        "Split test timed out waiting for results from subthreads!"

            match msg with
            | Go _ -> Expecto.Tests.failtest "Shouldn't receive the Go message twice!"
            | AllCompleted newParts ->
                let joinedT =
                    RRBVector.concat (
                        newParts
                        |> Seq.cast<RRBVector<int>>
                    )
                ...
                let joined =
                    ...
                    then
                        RRBVectorProps.checkProperties
                            joinedT
                            "Combined transient result of split test"
