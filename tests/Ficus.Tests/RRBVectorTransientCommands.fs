...
module internal VecCommands =
    ...
    let genInsert =
        Gen.map2 (fun x y -> x, y) (Gen.choose (-100, 100)) (Gen.choose (1, 100))
        |> Gen.map insert
    ...
    let slice (start, stop) =
        { new Cmd() with
            ...
            override __.Post(vec, arr) =
                vecEqual vec arr
                <| sprintf "After slicing from %A to %A, vec != arr" start stop
        ...
        { new Cmd() with
            override this.RunActual vec =
                ...
                let vL' =
                    ...
                    |> List.fold
                        (fun vec cmd ->
                            if cmd.Pre(RRBVector.toArray vec) then
                                let result = cmd.RunActual vec in

                                RRBVectorProps.checkProperties
                                    result
                                    (sprintf "After %s on left half of %s" (cmd.ToString()) s)

                                result
                            else
                                vec
                        )
                        (vL :?> RRBTransientVector<_>)
                    ...
                    |> List.fold
                        (fun vec cmd ->
                            if cmd.Pre(RRBVector.toArray vec) then
                                ...
                                RRBVectorProps.checkProperties
                                    result
                                    (sprintf "After %s on right half of %s" (cmd.ToString()) s)

                                result
                            else
                                vec
                        )
                        (vR :?> RRBTransientVector<_>)
                ...
                RRBVectorProps.checkProperties
                    result
                    (sprintf "After appending %A to %A at the end of %s" reprL reprR s)
            ...
            override __.RunModel arr =
                ...
                let arrL' =
                    cmdsL
                    |> List.fold (fun arr cmd -> if cmd.Pre arr then cmd.RunModel arr else arr) arrL

                let arrR' =
                    cmdsR
                    |> List.fold (fun arr cmd -> if cmd.Pre arr then cmd.RunModel arr else arr) arrR
            ...
            override __.Post(vec, arr) =
                vecEqual vec arr
                <| sprintf
                    "After splitting at %d and running %A on the left and %A on the right, vec != arr"
                    idx
                    cmdsL
                    cmdsR

            override __.ToString() =
                sprintf "split (%d,%A,%A)" idx cmdsL cmdsR
        }
    ...
    let genCmdsForSplit =
        Gen.sized (fun s ->
            Gen.listOfLength
                (s / 10
                 |> max 2)
                (Gen.frequency cmdFrequenciesForSplit)
        )

    let genSplit =
        Gen.map3
            (fun idx cmdsL cmdsR -> idx, cmdsL, cmdsR)
            (Gen.choose (-100, 100))
            genCmdsForSplit
            genCmdsForSplit
        |> Gen.map split
...
let internal cmdFrequenciesForComplexOperations =
    (1, genSplit)
    :: cmdFrequenciesForSplit

let expectedSize (vec: RRBVector<'a>) =
    if vec.Length > 200 then
        vec.Length
        / 100
        + (if vec.Length % 100 = 0 then 0 else 1)
    else
        4

let internal genBasicOperations (lst: Cmd list) =
    gen {
...
let internal genComplexOperations: Gen<RRBVector<int> * (Cmd list)[]> =
    gen {
        let! vec = RRBVectorGen.sizedGenVec
        // let vec = RRBVector.empty<int>
        let size = expectedSize vec
        ...
        let! cmds =
            Gen.arrayOfLength size (Gen.listOf (Gen.frequency cmdFrequenciesForComplexOperations))
...
let doTestXL (vec: RRBVector<int>) =
    ...
    propFromCmds vec cmdsExtraLarge
    |> Command.toProperty
...
let doComplexTest (vec: RRBVector<int>) =
    ...
    propFromCmdFrequencies vec cmdFrequenciesForComplexOperations
    |> Command.toProperty
