...
module RRBMath =
    ...
    let findMergeCandidatesTwoPasses (sizeSeq: #seq<int>) len =
        ...
        if len2 < (len1 * 2) then idx2, len2, 2 else idx1, len1, 1
...
type RRBNode<'T>(ownerToken: OwnerToken) =
    ...
    member this.IsEditableBy(owner: OwnerToken) =
        isSameObj owner ownerToken
        && not (isNull !owner)
    // Note that this test is NOT "if owner = owner".

    static member PopulateSizeTableS
        (shift: int)
        (array: RRBNode<'T>[])
        (len: int)
        (sizeTable: int[])
    ...
    static member MkLeaf (owner: OwnerToken) (items: 'T[]) =
    ...
    abstract member InsertedTree:
        OwnerToken -> int -> int -> 'T -> RRBFullNode<'T> option -> int -> SlideResult<RRBNode<'T>> // Params: owner shift treeIdx (item : 'T) (parentOpt : Node option) idxOfNodeInParent
...
and [<StructuredFormatDisplay("FullNode({StringRepr})")>] RRBFullNode<'T>
    (ownerToken: OwnerToken, children: RRBNode<'T>[]) =
    inherit RRBNode<'T>(ownerToken)

    static member Create(ownerToken: OwnerToken, children: RRBNode<'T>[]) =
        RRBFullNode<'T>(ownerToken, children)
    ...
    member this.LastChild =
        this.Children.[this.NodeSize
                       - 1]

    // Used in merging
    member this.LeftmostTwig shift =
        if shift > Literals.shiftSize then
            (this.FirstChild :?> RRBFullNode<'T>).LeftmostTwig(down shift)
        else
            this

    member this.RightmostTwig shift =
        if shift > Literals.shiftSize then
            (this.LastChild :?> RRBFullNode<'T>).RightmostTwig(down shift)
        else
            this
    ...
    override this.TreeSize shift =
        ...
        ((this.NodeSize
          - 1)
         <<< shift)
        + this.LastChild.TreeSize(down shift)

    override this.SlotCount =
        this.Children
        |> Array.sumBy (fun child -> child.NodeSize)
    ...
    override this.GetEditableNode owner =
        if this.IsEditableBy owner then
            ...
            RRBFullNode<'T>(owner, Array.copy this.Children) :> RRBNode<'T>
    ...
    override this.ShrinkRightSpineOfChild owner shift =
        ...
        then
            ...
            let child' =
                (this.LastChild :?> RRBFullNode<'T>).ShrinkRightSpine owner (down shift)
            ...
            if isSameObj child' this.LastChild then
                ...
                (node' :?> RRBFullNode<'T>).Children.[node'.NodeSize
                                                      - 1] <- child'
    ...
    member this.ExpandRightSpine owner shift =
        ...
        then
            ...
            let child' =
                (this.LastChild :?> RRBFullNode<'T>).ExpandRightSpine owner (down shift)
    ...
    member this.ReplaceLastLeaf owner shift (newLeaf: RRBLeafNode<'T>) sizeDiff =
        ...
        then
            ...
            let lastChild' =
                (this.LastChild :?> RRBFullNode<'T>).ReplaceLastLeaf
                    owner
                    (down shift)
                    newLeaf
                    sizeDiff
    ...
    default this.ToRelaxedNodeIfNeeded shift =
        if shift <= 0 then
            ...
            if isSizeTableFullAtShift shift sizeTable sizeTable.Length then
                ...
                RRBRelaxedNode<'T>(ownerToken, this.Children, sizeTable) :> RRBNode<'T>
    ...
    member this.ChildrenSeq =
        this.Children
        |> Seq.truncate this.NodeSize
    ...
    member this.LeavesSeq shift =
        ...
        then
            ...
            this.ChildrenSeq
            |> Seq.collect (fun child -> (child :?> RRBFullNode<'T>).LeavesSeq(down shift))
    ...
    member this.RevLeavesSeq shift =
        ...
        then
            ...
            this.ChildrenSeq
            |> Seq.rev
            |> Seq.collect (fun child -> (child :?> RRBFullNode<'T>).RevLeavesSeq(down shift))
    ...
    default this.InsertChild owner shift localIdx newChild =
        if localIdx = this.NodeSize then
            ...
            let newChildren =
                this.Children
                |> Array.copyAndInsertAt localIdx newChild
    ...
    default this.RemoveChild owner shift localIdx =
        let newChildren =
            this.Children
            |> Array.copyAndRemoveAt localIdx
    ...
    abstract member SplitAndKeepNLeftS:
        OwnerToken -> int -> int -> (RRBNode<'T> * (RRBNode<'T>[] * int[]))
    ...
    abstract member SplitAndKeepNRightS:
        OwnerToken -> int -> int -> ((RRBNode<'T>[] * int[]) * RRBNode<'T>)
    ...
    abstract member AppendNChildren:
        OwnerToken -> int -> int -> RRBNode<'T> seq -> bool -> RRBNode<'T>
    ...
    abstract member AppendNChildrenS:
        OwnerToken -> int -> int -> RRBNode<'T> seq -> int seq -> bool -> RRBNode<'T>
    ...
    abstract member PrependNChildrenS:
        OwnerToken -> int -> int -> RRBNode<'T> seq -> int seq -> RRBNode<'T>
    ...
    default this.MaybeExpand owner shift = this :> RRBNode<'T>
        ...
        Array.init
            ...
            (fun idx ->
                if idx = lastIdx then
                    fullSize
                    * idx
                    + this.Children.[idx].TreeSize(down shift)
                else
                    fullSize
                    * (idx + 1)
            )
    ...
    member this.NewPathL<'T> owner endShift (leaf: RRBLeafNode<'T>) =
        ...
        let rec loop shift node =
            ...
            then
                ...
                let shift' = (up shift) in loop shift' (RRBNode<'T>.MkNode owner shift' [| node |])
    ...
    member this.NewPathR<'T> owner endShift (leaf: RRBLeafNode<'T>) =
        ...
        let rec loop shift (node: RRBFullNode<'T>) =
            if
                ...
                >= endShift
            then
                node.MaybeExpand owner shift
            else
                let shift' = (up shift) in

                loop
                    shift'
                    (this.NewParent owner shift' [| node.MaybeExpand owner shift |]
                    :?> RRBFullNode<'T>)

        loop
            Literals.shiftSize
            (this.NewParent owner Literals.shiftSize [| leaf |] :?> RRBFullNode<'T>)
    ...
    member this.TryAppendLeaf owner shift (newLeaf: RRBLeafNode<'T>) leafLen =
        ...
        then
            ...
            then
                ...
                this.AppendChildS owner shift newLeaf leafLen
                |> Some
        else
            match
                (this.LastChild :?> RRBFullNode<'T>).TryAppendLeaf
                    owner
                    (down shift)
                    newLeaf
                    leafLen
            ...
            | None -> // Child's subtree had no room to append leaf
                ...
                then
                    ...
                    this.AppendChildS owner shift (this.NewPathR owner (down shift) newLeaf) leafLen
                    |> Some
    ...
    member this.AppendLeaf owner shift (newLeaf: RRBLeafNode<'T>) =
        ...
        | None ->
            ...
            let newParent =
                this.NewParent owner shift [|
                    this.ShrinkRightSpine owner shift
                    newRight
                |]
    ...
    member this.TryPrependLeaf owner shift (newLeaf: RRBLeafNode<'T>) leafLen =
        ...
        then
            ...
            then
                ...
                this.InsertChildS owner shift 0 newLeaf leafLen
                |> Some
        else
            match
                (this.FirstChild :?> RRBFullNode<'T>).TryPrependLeaf
                    owner
                    (down shift)
                    newLeaf
                    leafLen
            with
            | Some newChild ->
                this.UpdateChildSRel owner shift 0 newChild leafLen
                |> Some
            | None -> // Child's subtree had no room to prepend leaf
                ...
                then
                    ...
                    this.InsertChildS
                        ...
                        shift
                        0
                        (this.NewPathL owner (down shift) newLeaf)
                        leafLen
                    |> Some
    ...
    member this.SlideChildrenLeft owner shift n (leftSibling: RRBFullNode<'T>) =
        ...
        if n = 0 then
            leftSibling :> RRBNode<'T>, this :> RRBNode<'T>
        else
            let keepCnt =
                this.NodeSize
                - n
    ...
    member this.SlideChildrenRight owner shift n (rightSibling: RRBFullNode<'T>) =
        if n = 0 then
            this :> RRBNode<'T>, rightSibling :> RRBNode<'T>
        else
            let keepCnt =
                this.NodeSize
                - n
    ...
    member this.InsertAndSlideChildrenLeft
        owner
        shift
        localIdx
        newChild
    ...
    member this.InsertAndSlideChildrenRight
        owner
        shift
        localIdx
        newChild
        ...
        if
            thisSize = Literals.blockSize
            && localIdx = Literals.blockSize
            && sibSize = Literals.blockSize
                         - 1
        then
    ...
    member this.InsertAndSplitNode owner shift localIdx newChild =
        ...
        if shouldInsertIntoLeft then
            ...
            let rNode' =
                (rNode :?> RRBFullNode<'T>).InsertChild
                    owner
                    shift
                    (localIdx
                     - half)
    ...
    override this.InsertedTree owner shift treeIdx item parentOpt idxOfNodeInParent =
        ...
        let insertResult =
            child.InsertedTree owner (down shift) nextLvlIdx item (Some this) localIdx
            ...
            let node' =
                this.UpdateChildSAbs
                    ...
                    (localIdx
                     - 1)
                    newLeftChild
                    (newLeftChild.TreeSize(down shift))
                :?> RRBFullNode<'T>

            SimpleInsertion(
                node'.UpdateChildSAbs owner shift localIdx newChild (newChild.TreeSize(down shift))
            )
        ...
        | SlidItemsRight(newChild, newRightChild) ->
            let newNode =
                this.UpdateChildSAbs owner shift localIdx newChild (newChild.TreeSize(down shift))
                :?> RRBFullNode<'T>

            SimpleInsertion(
                newNode.UpdateChildSAbs
                    owner
                    shift
                    (localIdx
                     + 1)
                    newRightChild
                    (newRightChild.TreeSize(down shift))
            )
        ...
        | SplitNode(newLeftChild, newRightChild) ->
            if this.NodeSize < Literals.blockSize then
                ...
                | Some parent, idx when
                    idx > 0
                    && parent.Children.[idx - 1].NodeSize < Literals.blockSize
                    ->
                    ...
                    let newLeft, newRight =
                        newNode.InsertAndSlideChildrenLeft
                            owner
                            shift
                            (localIdx
                             + 1)
                ...
                | Some parent, idx when
                    ...
                    let newLeft, newRight =
                        newNode.InsertAndSlideChildrenRight
                            owner
                            shift
                            (localIdx
                             + 1)
    ...
    override this.RemovedItem owner shift shouldCheckForRebalancing treeIdx =
        ...
        let child' =
            child.RemovedItem owner (down shift) shouldCheckForRebalancing nextTreeIdx
...
*)
        ...
        if nextTreeIdx = 0 then
            ...
            if localIdx = 0 then
                failwith
                    "In KeepNTreeItems, localIdx should never be 0 when nextTreeIdx = 0, because that means we should have stopped at a level further up"

            if localIdx < 0 then
                failwith "In KeepNTreeItems, localIdx should never be < 0"
#endif
...
*)
    ...
    override this.SkipNTreeItems owner shift treeIdx =
        ...
        if nextTreeIdx = 0 then
            ...
            if localIdx = 0 then
                failwith
                    "In SkipNTreeItems, localIdx should never be 0 when nextTreeIdx = 0, because that means we should have stopped at a level further up"

            if localIdx < 0 then
                failwith "In SkipNTreeItems, localIdx should never be < 0"
#endif
    ...
    override this.SplitTree owner shift treeIdx =
        ...
        if nextTreeIdx = 0 then
            ...
            if localIdx = 0 then
                failwith
                    "In SplitTree, localIdx should never be 0 when nextTreeIdx = 0, because that means we should have stopped at a level further up"

            if localIdx < 0 then
                failwith "In SplitTree, localIdx should never be < 0"
#endif
            ...
            let rightNode' =
                (rightNode :?> RRBFullNode<'T>).UpdateChildSAbs
                    owner
                    shift
                    0
                    childR
                    (childR.TreeSize(down shift))
    ...
    member this.RemoveLastLeaf owner shift =
        ...
        then
            ...
            // Children are nodes
            let leaf, newLastChild =
                (this.LastChild :?> RRBFullNode<'T>).RemoveLastLeaf owner (down shift)

            let newNode =
                if newLastChild.NodeSize = 0 then
                    this.RemoveLastChild owner shift // Child had just one child of its own and is now empty, so remove it
                else
                    this.UpdateChildSAbs
                        owner
                        shift
                        (this.NodeSize
                         - 1)
                        newLastChild
                        (newLastChild.TreeSize(down shift))
    ...
    member this.Rebalance2Plus1
        ...
        let totalLength, childrenSeq =
            ...
            | None ->
                ...
                Seq.append
                    (left.Children
                     |> Seq.truncate left.NodeSize)
                    (right.Children
                     |> Seq.truncate right.NodeSize)
            | Some leaf ->
                left.NodeSize
                + 1
                + right.NodeSize,
    ...
    member this.RebalanceImpl
        (owner: OwnerToken)
        (shift: int)
        (totalLength: int)
        (childrenSeq: RRBNode<'T> seq)
        =
        let sizes =
            childrenSeq
            |> Seq.map (fun node -> node.NodeSize)
        ...
        then
            ...
            arr
            |> Array.fillFromEnumerator items 0 idx

            let mergedChildrenSeq =
                this.ApplyRebalancePlan owner shift sizes (idx, mergeLen, sizeReduction) items
            ...
            this.MkNodeForRebalance owner shift arrL lenL,
            (this.MkNodeForRebalance owner shift arrR lenR
             |> Some)
    ...
    member this.ApplyRebalancePlanImpl<'C>
        sizes
        (mergeStart, mergeLen, sizeReduction)
        (childrenEnum: System.Collections.Generic.IEnumerator<RRBNode<'T>>)
        ...
        let arraysSeq =
            Array.createManyFromSeq grandChildrenSeq totalSlots Literals.blockSize
    ...
    member this.ApplyRebalancePlan
        owner
        shift
        sizes
        (mergeStart, mergeLen, sizeReduction)
        ...
        =
        if shift > Literals.shiftSize then
            this.ApplyRebalancePlanImpl<RRBNode<'T>>
                sizes
                (mergeStart, mergeLen, sizeReduction)
                childrenEnum
                (fun (node: RRBNode<'T>) -> (node :?> RRBFullNode<'T>).SafeChildrenArr)
                (RRBNode<'T>.MkNode owner (down shift))
        else
            this.ApplyRebalancePlanImpl<'T>
                sizes
                (mergeStart, mergeLen, sizeReduction)
                childrenEnum
                (fun (leaf: RRBNode<'T>) -> (leaf :?> RRBLeafNode<'T>).Items)
                (RRBNode<'T>.MkLeaf owner)

    member this.ConcatNodes owner shift (right: RRBFullNode<'T>) =
        if this.NeedsRebalance2 shift right then
            this.Rebalance2 owner shift right
        elif
            this.NodeSize
        ...
        then
            ...
            let left' =
                ...
                if right :? RRBRelaxedNode<'T> then
                    this.AppendNChildrenS
                        owner
                        shift
                        right.NodeSize
                        right.Children
                        (right :?> RRBRelaxedNode<'T>).SizeTable
                        false
                else
    ...
    member this.ConcatTwigsPlusLeaf owner shift (middle: RRBLeafNode<'T>) (right: RRBFullNode<'T>) =
        ...
        then
            ...
            arr
            |> Array.fillFromSeq items 0 newLen
    ...
    member this.MergeTree
        owner
        shift
        (tailOpt: RRBLeafNode<'T> option)
        rightShift
        (right: RRBFullNode<'T>)
        shouldKeepExpandedLeftNode
        =
        let shrinkLeftNode
            owner
            shift
            shouldKeepExpandedLeftNode
            (pair: RRBNode<'T> * RRBNode<'T> option)
            =
            match pair with
        ...
        then
            ...
            | child', None ->
                ...
                if right.NodeSize > 1 then
                    ...
                    (parentL :?> RRBFullNode<'T>).ConcatNodes
                        owner
                        shift
                        (parentR :?> RRBFullNode<'T>)
                    |> shrinkLeftNode owner shift shouldKeepExpandedLeftNode
                else
                    (parentL, None)
                    |> shrinkLeftNode owner shift shouldKeepExpandedLeftNode
            | childL', Some childR' ->
                ...
                let childR' =
                    if right.NodeSize > 1 then
                        (childR' :?> RRBFullNode<'T>).ShrinkRightSpine owner (down rightShift)
                    else
                        childR'

                let parentR =
                    right.UpdateChildSAbs
                        owner
                        rightShift
                        0
                        childR'
                        (childR'.TreeSize(down rightShift))
            ...
            | child', None ->
                let child' =
                    if right.NodeSize > 1 then
                        (child' :?> RRBFullNode<'T>).ShrinkRightSpine owner (down rightShift)
                    else
                        child'
                ...
                let parentR =
                    right.UpdateChildSAbs
                        ...
                        0
                        child'
                        (child'.TreeSize(down rightShift))
...
and [<StructuredFormatDisplay("RelaxedNode({StringRepr})")>] RRBRelaxedNode<'T>
    (ownerToken: OwnerToken, children: RRBNode<'T>[], sizeTable: int[]) =
    ...
    static member CreateWithSizeTable
        (owner: OwnerToken, shift: int, children: RRBNode<'T>[], sizeTbl: int[])
        =
    ...
    override this.GetEditableNode owner =
        if this.IsEditableBy owner then
            ...
            RRBRelaxedNode<'T>(owner, Array.copy this.Children, Array.copy this.SizeTable)
            :> RRBNode<'T>
    ...
    default this.ToFullNodeIfNeeded shift =
        if isSizeTableFullAtShift shift this.SizeTable this.SizeTable.Length then
            RRBFullNode<'T>(this.Owner, this.Children) :> RRBNode<'T>
        else
            this :> RRBNode<'T>
    ...
    override this.AppendChildS owner shift newChild newChildSize =
        ...
        let lastSizeTableEntry =
            if this.SizeTable.Length = 0 then
                0
            else
                Array.last this.SizeTable
    ...
    override this.RemoveChild owner shift localIdx =
        ...
        for i = localIdx to this.NodeSize
                            - 2 do
            sizeTable'.[i] <-
                sizeTable'.[i]
                - oldChildSize
...
and [<StructuredFormatDisplay("ExpandedFullNode({StringRepr})")>] RRBExpandedFullNode<'T>
    (ownerToken: OwnerToken, children: RRBNode<'T>[], ?realSize: int) =
    ...
    override this.SlotCount =
        this.Children
        |> Seq.take this.NodeSize
        |> Seq.sumBy (fun child -> child.NodeSize)

    override this.SafeChildrenArr =
        this.Children
        |> Array.truncate this.NodeSize
    ...
    override this.Shrink owner =
        ...
        if
            this.IsEditableBy owner
            && size = Literals.blockSize
        then
            RRBFullNode<'T>(owner, this.Children) :> RRBNode<'T>
        else
            let children' =
                if size = Literals.blockSize then
                    Array.copy this.Children
                else
                    this.Children
                    |> Array.truncate size
    ...
    override this.GetEditableNode owner =
        if this.IsEditableBy owner then
            ...
            RRBExpandedFullNode<'T>(owner, Array.copy this.Children, this.NodeSize) :> RRBNode<'T>
    ...
    override this.ToRelaxedNodeIfNeeded shift =
        if shift <= 0 then
            ...
            if isSizeTableFullAtShift shift sizeTable size then
                this :> RRBNode<'T>
            else
                RRBExpandedRelaxedNode<'T>(ownerToken, this.Children, sizeTable, this.NodeSize)
                :> RRBNode<'T>
    ...
    override this.AppendChild owner shift newChild =
        ...
        if trulyFull then
            ...
            sizeTable'.[oldSize] <-
                lastEntry
                + newChild.TreeSize(down shift)

            RRBExpandedRelaxedNode<'T>(owner, node'.Children, sizeTable', node'.NodeSize)
            :> RRBNode<'T>
    ...
    override this.RemoveChild owner shift localIdx =
        if
            localIdx = this.NodeSize
                       - 1
        then
            this.RemoveLastChild owner shift
        else
            let node' = this.GetEditableNode owner :?> RRBExpandedFullNode<'T>
    ...
    override this.UpdateChildSRel owner shift localIdx newChild sizeDiff =
        ...
        then
            ...
            let node' =
                RRBExpandedRelaxedNode<'T>(this.Owner, this.Children, sizeTable, this.NodeSize)
    ...
    override this.AppendNChildren owner shift n newChildren shouldExpandRightChildIfNeeded =
        ...
        let mutable stillFull =
            (node'.LastChild.TreeSize(down shift)
             >= fullChildSize)
    ...
    override this.PrependNChildren owner shift n newChildren =
        ...
        for i = 0 to n - 1 do
            if eC.MoveNext() then
                ...
                if childSize < fullChildSize then
                    stillFull <- false
    ...
    override this.MkNodeForRebalance owner shift arr len =
        if
            isSameObj arr this.Children
            && this.IsEditableBy owner
        then
            for i = len to this.NodeSize
                           - 1 do
...
and [<StructuredFormatDisplay("ExpandedRelaxedNode({StringRepr})")>] RRBExpandedRelaxedNode<'T>
    (ownerToken: OwnerToken, children: RRBNode<'T>[], sizeTable: int[], ?realSize: int) =
    inherit
        RRBRelaxedNode<'T>(
            ownerToken,
            Array.expandToBlockSize children,
            Array.expandToBlockSize sizeTable
        )
    ...
    override this.ToFullNodeIfNeeded shift =
        if isSizeTableFullAtShift shift this.SizeTable this.NodeSize then
            RRBExpandedFullNode<'T>(this.Owner, this.Children, this.NodeSize) :> RRBNode<'T>
        else
            this :> RRBNode<'T>
...
and [<StructuredFormatDisplay("{StringRepr}")>] RRBLeafNode<'T>(ownerToken: OwnerToken, items: 'T[])
    ...
    override this.GetEditableNode owner =
        if this.IsEditableBy owner then
            ...
            RRBLeafNode<'T>(owner, Array.copy items) :> RRBNode<'T>

    member this.GetEditableArrayOfBlockSizeLength owner =
        if this.IsEditableBy owner then
            this.Items
            |> Array.expandToBlockSize
        else
    ...
    override this.UpdatedTree owner shift treeIdx newItem = this.UpdatedItem owner treeIdx newItem
    ...
    member this.InsertedItem owner localIdx item =
        ...
        let newItems =
            this.Items
            |> Array.copyAndInsertAt localIdx item
        ...
        if this.NodeSize < Literals.blockSize then
            ...
            | Some parent, idx when
                ...
                let newLeftItems, newRightItems =
                    Array.appendAndInsertAndSplitEvenly
                        (localIdx
                         + leftSib.NodeSize)
                        item
                        leftSib.Items
            ...
            | Some parent, idx when
                ...
                let newLeftItems, newRightItems =
                    Array.appendAndInsertAndSplitEvenly localIdx item this.Items rightSib.Items
            ...
            | _ ->
                ...
                let newLeftItems, newRightItems =
                    Array.insertAndSplitEvenly localIdx item this.Items
    ...
    override this.GetTreeItem _shift localIdx = this.Items.[localIdx]
    ...
    override this.RemovedItem owner shift shouldCheckForRebalancing localIdx =
        Array.copyAndRemoveAt localIdx this.Items
        |> RRBNode<'T>.MkLeaf owner

    override this.KeepNTreeItems owner shift treeIdx =
        this.Items
        |> Array.truncate treeIdx
        |> RRBNode<'T>.MkLeaf owner
