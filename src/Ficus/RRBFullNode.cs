...
public class RRBFullNode<T> : RRBNode<T>
{
    ...
    public override RRBNode<T> GetEditableNode(OwnerToken owner)
    {
        ...
        // TODO: See if the following is allowed (will it upcast automatically?)
        // return new RRBFullNode<T>(owner, Children.Clone());
    }
    ...
    // TODO: Implement KeepNLeft, KeepNRight, SplitAndKeepNLeft(S), SplitAndKeepNRight(S), AppendNChildren(S), PrependNChildren(S), MaybeExpand
    ...
    public virtual ((RRBNode<T>[] leftChildren, int[] leftSizeTable), RRBNode<T> node) SplitAndKeepNRightS(OwnerToken owner, int shift, int n)
    {
        ...
        var lS = new int[skip];
        var rS = new int[n];
        Array.Copy(sizeTable, 0, lS, 0, skip);
        Array.Copy(sizeTable, skip, rS, 0, n);
    ...
    // TODO: Write specific tests for InsertAndSlideChildren (left/right) and InsertAndSplitNode methods, because they're fundamental and I want to make sure all corner cases get covered
    ...
    public (RRBNode<T> left, RRBNode<T>? right) RebalanceImpl(OwnerToken owner, int shift, int totalLength, IEnumerable<RRBNode<T>> childrenSeq)
    {
        ...
        using (var items = childrenSeq.GetEnumerator())
        {
            ...
            else
            {
                ...
                RRBArrayExtensions.RRBArrayExtensions.FillFromEnumerator(arrL, items, 0, idx);
                RRBArrayExtensions.RRBArrayExtensions.FillFromEnumerator(arrR, items, 0, idx);

                var mergedChildrenSeq = ApplyRebalancePlan(owner, shift, sizes, (idx, mergeLen, sizeReduction), items);

                RRBArrayExtensions.RRBArrayExtensions.FillFromSeq(arrL, mergedChildrenSeq, idx, mergeLen - sizeReduction);
                RRBArrayExtensions.RRBArrayExtensions.FillFromSeq(arrR, mergedChildrenSeq, idx, mergeLen - sizeReduction);

                RRBArrayExtensions.RRBArrayExtensions.FillFromEnumerator(arrL, items, idx + mergeLen - sizeReduction, lenL);
                RRBArrayExtensions.RRBArrayExtensions.FillFromEnumerator(arrR, items, idx + mergeLen - sizeReduction, lenR);
    ...
    public IEnumerable<RRBNode<T>> ApplyRebalancePlanImpl<C>(
        IEnumerable<int> sizes,
        (int, int, int) mergePlan,
        IEnumerator<RRBNode<T>> childrenEnum,
        Func<RRBNode<T>, C[]> getGrandChildren,
        Func<C[], RRBNode<T>> mkChildNode)
    {
        var totalSlots = sizes.Skip(mergePlan.Item1).Take(mergePlan.Item2).Sum();

        // TODO: See if there is a speedup to be had by making an enumerable here instead of a List()
        var grandChildrenSeq = new List<C>();
        while (childrenEnum.MoveNext())
        {
            var child = childrenEnum.Current;
            grandChildrenSeq.AddRange(getGrandChildren(child));
        }
        // The F# original had `seq ( ... yield! getGrandChildren child }`, which was efficient. This is far less efficient.
        // TODO: Speed this up, wrtiing tests to ensure it stays correct, then benchmark
        // BENCHMARK - search for this uppercase text to find benchmarking opportunities

        var arraysSeq = RRBArrayExtensions.RRBArrayExtensions.CreateManyFromSeq(grandChildrenSeq, totalSlots, Literals.blockSize);
        return arraysSeq.Select(mkChildNode);
    }
    ...
    public IEnumerable<RRBNode<T>> ApplyRebalancePlan(
        ...
        IEnumerator<RRBNode<T>> childrenEnum)
    {
        // TODO: Rewrite this without an Impl in there, because in C# the efficiency gain of `seq { ... yield! }` has been lost
        // There ought to be a way to write this imperitavely to gain some efficiency
        if (shift > Literals.shiftSize)
        {
            // We are above twig level, children are nodes
            return ApplyRebalancePlanImpl<RRBNode<T>>(
                sizes,
                mergePlan,
                childrenEnum,
                (node) => ((RRBFullNode<T>)node).SafeChildrenArr,
                (arr) => RRBNode<T>.MkNode(owner, RRBMath.Down(shift), arr));
        }
        else
        {
            // We are at twig level, children are leaves
            return ApplyRebalancePlanImpl<T>(
                sizes,
                mergePlan,
                childrenEnum,
                (leaf) => ((RRBLeafNode<T>)leaf).Items,
                (arr) => RRBNode<T>.MkLeaf(owner, arr));
        }
    }
