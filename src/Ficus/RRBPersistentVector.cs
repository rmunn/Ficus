...
internal sealed class RRBPersistentVector<T> : RRBVector<T>
{
    ...
    internal RRBVector<T> ShiftNodesFromTailIfNeeded()
    {
        ...
        if (shiftCount >= tailLen)
        {
            ...
            var newTail = removedLeaf.Items.Append(Tail);
    ...
    private RRBPersistentVector<T> AppendPersistent(RRBPersistentVector<T> right)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // (mergedRoot, maybeNewRight) = ((RRBFullNode<T>)this.Root).MergeTree(OwnerTokens.NullOwner, newLeftShift, null, right.Shift, (RRBFullNode<T>)right.Root, false); // buggy
                (mergedRoot, maybeNewRight) = ((RRBFullNode<T>)newLeftRoot).MergeTree(OwnerTokens.NullOwner, newLeftShift, null, right.Shift, (RRBFullNode<T>)right.Root, false); // bugfix
            }
